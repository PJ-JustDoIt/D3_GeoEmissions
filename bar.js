...
//highlight current year with different color vs other years
function highlightBars(year) {
  d3.select("#bar")
    .selectAll("rect")
      .attr("fill", d => d.year === year ? "#F1A409" : "#BFBD11");
}
...
function drawBar(data, dataType, country) {
  ...
  var axisLabel = dataType === "emissions" ?
    "Greenhouse emissions, 1000 metric tons" :
    "Greenhouse emissions, metric tons per capita";

  var barTitle = country ?
    "Greenhouse Gas Emissions, " + country :
    "Click on a country in the map to see bar chart of annual trends.";
  ...
  update
    ...
    .append("rect")
      ...
      .attr("height", 0)
    .merge(update)
      .attr("x", d => (xScale(d.year) + xScale(d.year - 1)) / 2+barWidth/2)
      .attr("width", barWidth - 50*barPadding)
      .transition(t)
      .delay((d, i) => i * 100)
