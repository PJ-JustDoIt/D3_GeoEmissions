...
d3.queue()
  .defer(d3.json, "//unpkg.com/world-atlas@1.1.4/world/50m.json")    // for map
  ...
  .await(function(error, mapData, data) {
    ...
    var currentDataType = d3.select('input[name="data-type"]:checked')    
                            .attr("value");
    var geoData = topojson.feature(mapData, mapData.objects.countries).features;   // topojson to geo conversion
    ...
    // draw maps etc based on input year change 
    d3.select("#year")
    ...
    //draw mapss etc based on radio button choice
    d3.selectAll('input[name="data-type"]')
        .on("change", () => {
          var active = d3.select(".active").data()[0];
          var country = active ? active.properties.country : "";
          currentDataType = d3.event.target.value;
          // pie chart only for emissions , not for emissions per capita
          drawMap(geoData, data, currentYear, currentDataType);
